...
void	ParseRequestLine::assignValueToField(const std::string& value, const int option, const std::vector<std::string>& vec)
{
...
int	ParseRequestLine::parseRequestLineField(size_t& pos, std::string& remaining_request_str, int option, const std::vector<std::string>& vec)
{
	std::string	value;
	...
	if (pos && pos != std::string::npos)
	{
		...
		assignValueToField(value, option, vec);
	}
	else
		return (-1);
	return (0);	
...
int	ParseRequestLine::parseRequestLine(const std::string& request, const std::vector<std::string>& vec)
{
	size_t		pos;
	...
	try
	{
		if (parseRequestLineField(pos, remaining_request_str, METHOD, vec))
			return (404);
		if (parseRequestLineField(pos, remaining_request_str, REQUEST_TARGET, vec))
			return (404);
