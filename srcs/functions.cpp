...
void	parseInput(t_objects& objs, int& code, const t_definitions& def)
{
...
//	if (!code)
//		code = objs.req_heads.parseHeaders(req.getHeaders());
}
...
//	parseHeadersOut(status);
	if (objs.res_body.parseBodyOut(status, objs.res, objs.res_status_line))
		return (-1);
	objs.res.parseResponse();
	return (0);
}

int	serverExecution(t_objects& objs, t_exec& exec)
{
	int	code;
	...
	{
		case GET:
			code = exec.get_method->execParticularMethod(objs.req_line, objs.res_body);
			break;
/*		case POST:
